    ...
    return ( <> { loading ?
        ...
        <div>
         <div className="w-full h-36 bg-[#b998ff] flex flex-col items-center justify-center mb-5">
                <div className="text-5xl font-bold font-oranienbaum">{data?.type} {data?.id}</div>
            ...
            <div className="flex justify-center m-3"><button className="w-56 h-[4rem] bg-[#9785df] text-2xl font-semibold border-[6px] border-black" onClick={()=>viewSlot()}>Im Interested</button></div>
            <WorkshopSlot slot={slot} setSlot={setSlot} data={slotData!}/>
            <Messages message={message} setMessage={setMessage} data="Sorry Slots are full" sucess={false}/>
        </div> }
    </>)
}
