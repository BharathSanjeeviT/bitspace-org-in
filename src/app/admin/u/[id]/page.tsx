...
const AdminProfile = ({ params }: { params: { id: string } }) => {
    ...
    return (
        <div className="flex justify-around">
            ...
            {show && (
                <div className="h-[86vh] w-[60%] rounded-xl border-4 border-black p-2 m-4">
                    <form onSubmit={handle} className="bg-[#9D8AE8] min-w-full h-full rounded-xl border border-3 border-black p-10">
                        ...
                        <div>
                            ...
                            <div className="pt-3 pb-3 bg-white border-4 border-black rounded">
                                {curr.Role.map((ele: any, i: any) => {
                                    return (
                                        <div className="p-2 border-2 border-black rounded-md w-[125px] font-bold inline m-1" key={i}>{ele.role.substr(0, 9) + ' '}{(<CancelOutlinedIcon onClick={(e) => handleClose(i)} />)}</div>
                                    )
                                })}
                            </div>
